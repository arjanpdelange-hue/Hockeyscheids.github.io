    ...
    <script>
        ...
        function startPauseMatch() {
            ...
            if (state.isRunning) {
                ...
                // 1. Log de officiële start van het 1e Kwart
                if (state.quarter === 1 && state.matchTimeSeconds === state.defaultQuarterDuration) {
                    logEvent(`OFFICIËLE WEDSTRIJDSTART: ${QUARTER_NAMES[1]}.`, 'start');
                }
        ...
        function generatePdf() {
            ...
            const body = state.timeline.map(item => {
                ...
                let icon = '';
                if (item.event.includes('Doelpunt')) {
                    icon = '🏑'; // Veldhockey stok en bal icoon
                } else if (item.event.includes('Groene Kaart')) {
                    icon = '▲'; 
                } else if (item.event.includes('Gele Kaart')) {
                    icon = '■'; 
                ...
                // Tekst opschonen
                if (item.event.includes('OFFICIËLE WEDSTRIJDSTART')) {
        ...
        function resetMatch() {
            ...
            // Reset de hele state naar de initiële waarden, met behoud van configuratie
            state = {
                quarter: 1,
        ...
        window.onload = function() {
            ...
            // Roep de update functie aan om de initiële staat te renderen
